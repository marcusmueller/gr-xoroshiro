 ...
 */
void xoroshiro128p_seed(uint64_t *state, const uint64_t seed) {
  state[0] = seed;
 ...
 */
float xoroshiro128p_cltf(uint64_t *state, const uint32_t iterations) {
  ...
  int64_t sum = 0;
  for(uint32_t _ = 0; _ < iterations; _++) {
    sum +=  (xoroshiro128p_next(state) >> 32) - mu_32;
  }
  sum += iterations / 2;
  float val = (float)sum / std_32 / sqrtf(iterations);
  return val;
}
